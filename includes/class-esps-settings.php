...
class ESPS_Settings {
    ...
    public function render_post_types_multiselect($args) {
        ...
        echo "<select id='post_types' class='esps-chosen' name='{$this->option_key}[post_types][]' multiple='multiple' style='width: 300px; max-width: 100%;'>";
        foreach ($post_types as $type) {
            $is_selected = in_array($type->name, $selected) ? 'selected' : '';
            echo "<option value='{$type->name}' {$is_selected}>{$type->label}</option>";
        }
        echo "</select>";
    }
    ...
    public function enqueue_admin_assets($hook) {
        ...
        wp_enqueue_script('chosen-js', 'https://cdn.jsdelivr.net/npm/chosen-js@1.8.7/chosen.jquery.min.js', ['jquery'], null, true);
        wp_add_inline_script('chosen-js', "jQuery(document).ready(function($) { $('.esps-chosen').chosen(); });");
    }
